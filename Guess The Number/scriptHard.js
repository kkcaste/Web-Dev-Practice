...
function guess(){
	if(usrInput.value>0&&usrInput.value<=100){
		...
		window.alert("Write a number between 1 and 100!!");
	}
}
function showTries(nTry){
	console.log("Try --> "+nTry);
	tryShower.innerHTML="You have "+(nTry)+" attempts";
	if(nTry==1){
...
function triesFinished(){
	...
	document.getElementById("bigOrSmall").hidden=true;
	tryShower.innerHTML="";
	tryShower.innerHTML="You have finished all of your attempts!!<br>The number was: "+randomNum+"<br>If you want to retry click <strong><i>Retry</i></strong>";
	tryShower.hidden=false;
	retryBtn.hidden=false;
	guessBtn.hidden=true;
	usrInput.disabled=true;
